...
file = 'THR_Taxonomy_Justin_20210526.csv'        ## SPECIFY filename
custid = 483
...
with open(file, 'r') as fr:
    ...
    for line in fr:
        ...
        taxid = elements[1].strip().replace('-', '')
        provider = elements[0].strip().replace(',', '').replace("'", "").replace('-', '')
        npi = elements[2].strip()
        taxon = elements[3].strip()
        #addr = elements[3].strip().replace('-', '')
        #phone = elements[4].strip().replace('(', '').replace(')', '').replace('-', '').replace(' ', '')

        k = '{}-{}-{}-{}'.format(taxid, provider, npi, taxon)
        print(k)
        dicy[k] = 1
...
for i in dicy:
    ...
    sql = """\
insert into
	business_analysis.enrollment_tracking (cust_id, billing_provider_taxid, billing_provider, billing_provider_npi, billing_provider_taxonomy)
values
	({}, '{}', '{}', nullif('{}', ''), nullif('{}', ''));""".format(custid, info[0], info[1], info[2], info[3])
    with con:
        cur = con.cursor()
        cur.execute(sql)
        print(sql)
